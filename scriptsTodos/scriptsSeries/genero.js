function exibirGenero() {
    fetch('https://api.themoviedb.org/3/tv/'+num+'/similar?language=pt-br&page=1', options)
    ...
    for (let index = 0; index < listaFilmesPopulares.length; index++) {
        ...
        divSeries.appendChild(divImagensPopular);
        ...
        botaoVerMais.addEventListener("click",(e)=>{
          ...
          if(listaFilmesPopulares[index].overview.length == 0) {
            descricaoModal.innerHTML = "Esta Série ainda não possui descrição traduzida..."
          }
      ...
      for (let index = 1; index < 10; index++) {
        ...
        botaoContador.addEventListener("click",(e)=>{
            ...
            fetch('https://api.themoviedb.org/3/tv/'+num+'/similar?language=pt-br&page='+ index, options)
            .then(response => response.json())
            .then(response => { 
            let listaFilmesPopulares = response.results;
            
            exibirFilmesGenero(divSeries,listaFilmesPopulares)
